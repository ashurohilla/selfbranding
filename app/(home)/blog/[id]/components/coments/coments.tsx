...
const Comments = ({ id , toggleCommentSection  }: { id: string , toggleCommentSection: any }) => {
  ...
  // Update form values when user, comment, or id changes
  useEffect(() => {
    if (user?.id) {
  ...
  useEffect(() => {
    ...
    submitComment();
  }, [shouldSubmit, formValues, supabase]);

  // Fetch comments
  useEffect(() => {
    const readcommnets = async () => {
      setLoading(true);
      try {
        const { data , error } = await supabase
          .from("blog_coments")
        ...
        <div className="border-2 px-2 py-2">
          ...
          <div className="flex pt-4 items-center justify-between gap-8">
            ...
            <Button
              ...
              disabled={isSubmitting || !coment.trim()}
            >
